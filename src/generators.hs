...
import Test.QuickCheck
    ( Gen, elements, frequency, generate, oneof, Arbitrary(arbitrary) )
import Test.QuickCheck.Random ( QCGen, mkQCGen )
import Seeding ( grow2SeedLists, useSeed )
...
genASTExpressions seed d nrParam ratio n = do 
  sequence [generate g | g <- rampedHalfNHalf (mkQCGen seed) d nrParam ratio n]